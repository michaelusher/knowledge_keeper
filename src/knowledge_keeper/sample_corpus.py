"""Demo corpus with deliberately planted issues, so a first run has something to find:
...
- Deployment runbook: backdated ~2.5 years (stale)
- Two docs that disagree about the database failover procedure (conflict)
- One doc with no author metadata (orphan)

Used by the app's "Load demo documents" button and scripts/make_sample_corpus.py.
"""
...
def make_docx_runbook(OUT: Path):
    import docx
...
def generate(out_dir) -> list[Path]:
    """Write the demo corpus into out_dir and return the created files."""
    out = Path(out_dir)
    out.mkdir(parents=True, exist_ok=True)
    make_pptx(out)
    make_docx_runbook(out)
    make_docx_dr(out)
    make_md_faq(out)
    return [
        out / "orderflow_onboarding.pptx",
        out / "deployment_runbook.docx",
        out / "disaster_recovery_plan.docx",
        out / "payments_faq.md",
    ]
